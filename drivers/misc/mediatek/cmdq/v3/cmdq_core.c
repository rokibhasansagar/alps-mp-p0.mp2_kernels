	...
	struct TaskStruct *pTask, s32 thread, u32 timeout_ms)
{
	...
	while (timeout_ms > 0) {
		...
		CMDQ_LOG("ready to verify pre-dump!!\n");
		if (pTask->scenario == CMDQ_SCENARIO_DISP_ESD_CHECK || CMDQ_SCENARIO_PRIMARY_DISP) {
			uint32_t *pcVA = NULL, pcPA = 0;
			uint32_t insts[4] = { 0 };
			char parsedInstruction[128] = { 0 };
			...
			if (pcVA) {
				...
				if (op == CMDQ_CODE_WFE) {
					...
					CMDQ_REG_SET32(CMDQ_SYNC_TOKEN_ID,
						eventID);
					regValue =
			...
			CMDQ_LOG("task:%p scen:%d is not matched\n", pTask, pTask->scenario);
		}
#endif
