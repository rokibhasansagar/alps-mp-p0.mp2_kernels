...
static int disp_validate_session_params(unsigned int session_id)
{
	if (DISP_SESSION_TYPE(session_id) > DISP_SESSION_MEMORY) {
		DISPERR("unsupported session(0x%x) !\n", session_id);
		return -1;
	}
	return 0;
}
...
static int _ioctl_wait_all_jobs_done(unsigned long arg)
{
	...
	if (disp_validate_session_params(session_id)) {
		DISPERR("%s:unsupported session(0x%x) !\n",  __func__, session_id);
		return -EINVAL;
	}
